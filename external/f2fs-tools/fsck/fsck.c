...
int f2fs_set_reg_bitmap(struct f2fs_sb_info *sbi, u32 blk)
{
	struct f2fs_fsck *fsck = F2FS_FSCK(sbi);

	return f2fs_set_bit(BLKOFF_FROM_MAIN(sbi, blk), fsck->reg_main_area_bitmap);
}

int f2fs_test_reg_bitmap(struct f2fs_sb_info *sbi, u32 blk)
{
	struct f2fs_fsck *fsck = F2FS_FSCK(sbi);

	return f2fs_test_bit(BLKOFF_FROM_MAIN(sbi, blk),
						fsck->reg_main_area_bitmap);
}
		...
		int enc_name)
{
	...
	if (ftype == F2FS_FT_DIR) {
		...
		if (ftype == F2FS_FT_REG_FILE)
			f2fs_set_reg_bitmap(sbi, blk_addr);
		f2fs_set_main_bitmap(sbi, blk_addr, CURSEG_WARM_DATA);
	}
	return 0;
}

int fsck_chk_orphan_node(struct f2fs_sb_info *sbi, int keep_new_orphan)
{
	u32 blk_cnt = 0;
	...
	new_blk = calloc(BLOCK_SZ, keep_new_orphan ? orphan_blkaddr : 1);
	ASSERT(new_blk);

	if (keep_new_orphan)
		fsck->new_orphan_blk = new_blk;
	...
	for (i = 0; i < orphan_blkaddr; i++) {
		...
		if (keep_new_orphan)
			new_blk = &fsck->new_orphan_blk[i + 1];
		else
...
void fsck_init(struct f2fs_sb_info *sbi)
{
	...
	fsck->reg_main_area_bitmap = calloc(fsck->main_area_bitmap_sz, 1);
	ASSERT(fsck->reg_main_area_bitmap != NULL);
...
static void __fix_checkpoint(struct f2fs_sb_info *sbi)
{
	...
	*((__le32 *)((unsigned char *)cp + CP_CHKSUM_OFFSET)) = cpu_to_le32(crc);
	...
	if (fsck->new_orphan_blk) {
		for (i = 0; i < orphan_blks; i++) {
			ret = dev_write_block(&fsck->new_orphan_blk[i], cp_blk_no++);
			ASSERT(ret >= 0);
		}
	} else {
...
void fsck_write_extra_flag_blk(struct f2fs_sb_info *sbi)
{
	struct f2fs_super_block *sb = F2FS_RAW_SUPER(sbi);
	struct f2fs_fsck *fsck = F2FS_FSCK(sbi);
	block_t extra_blk_no = le32_to_cpu(sb->cp_blkaddr) - 1;
	...
	res = dev_write_block(&fsck->extra_blk, extra_blk_no);
	if (res < 0)
...
int fsck_chk_need_fsck(struct f2fs_sb_info *sbi)
{
	...
	if (le32_to_cpu(extra_blk->need_fsck) == F2FS_SEC_FSCK_MAGIC) {
		MSG(0, "Info: found force FSCK flag!\n");
		return 1;
	}

	if (!is_set_ckpt_flags(cp, CP_UMOUNT_FLAG) && 
		(unsigned)(le32_to_cpu(extra_blk->spo_counter) + 1) >= FSCK_SPO_CNT_THR) {
		MSG(0, "Info: %d spo count reached\n", FSCK_SPO_CNT_THR);
		return 1;
	}
...
void fsck_free(struct f2fs_sb_info *sbi)
{
	...
	if (fsck->new_orphan_blk)
		free(fsck->new_orphan_blk);

	if (fsck->main_area_bitmap)
		free(fsck->main_area_bitmap);

	if (fsck->reg_main_area_bitmap)
		free(fsck->reg_main_area_bitmap);
