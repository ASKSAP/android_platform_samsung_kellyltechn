...
int sanity_check_ckpt(struct f2fs_sb_info *sbi)
{
	...
	if (!user_block_count || user_block_count >=
		...
		MSG(0, "\tWrong user_block_count(%llu)\n", user_block_count);
		return 1;
	}
...
static int f2fs_init_nid_bitmap(struct f2fs_sb_info *sbi)
{
	...
	if (!(c.func == SLOAD || c.func == FSCK || c.func == F2IMAGE))
		return 0;
...
void move_curseg_info(struct f2fs_sb_info *sbi, u64 from, int left)
{
	...
	for (i = 0; i < NO_CHECK_TYPE; i++) {
		...
		se = get_seg_entry(sbi, curseg->segno);
		if (se->valid_blocks != curseg->next_blkoff)
...
int f2fs_do_mount(struct f2fs_sb_info *sbi)
{
	...
	if (c.auto_fix || c.preen_mode) {
		...
		if (flag & CP_FSCK_FLAG ||
			...
			if (!(flag & CP_UMOUNT_FLAG)) {
				unsigned spo_cnt = le32_to_cpu(fsck->extra_blk.spo_counter);
				fsck->extra_blk.spo_counter = cpu_to_le32(spo_cnt + 1);
